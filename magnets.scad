...
module magnet_holder(height, wall_distance, wall_rounding, clearance) {
  color(INSIDE_ACCESSORIES_COLOR)
    difference() {
      ...
      holder_size = wall_distance + get_magnet_holder_radius();
      holder_rounding = [
        0,                         // X+Y+ (RIGHT+BACK)
        wall_rounding,             // X-Y+ (LEFT+BACK)
        0,                         // X-Y- (LEFT+FRONT)
        get_magnet_holder_radius() // X+Y- (RIGHT+FRONT)
      ];
      holder_rect_xy_offset = (wall_distance - get_magnet_holder_radius()) / 2
                              - get_magnet_holder_rounding_fix_offset();
      holder_circle_xy_offset = get_magnet_holder_rounding_fix_offset();
        ...
        linear_extrude(height = holder_height, center = true)
          if (wall_rounding > holder_size) {
            move([holder_circle_xy_offset, -holder_circle_xy_offset])
              circle(r = get_magnet_holder_radius());
          } else {
      ...
      if (!clearance) {
        move([get_magnet_holder_rounding_fix_offset(),
              -get_magnet_holder_rounding_fix_offset(),
              holder_height
                - get_magnet_hole_height()
                - (magnet_generate_closure ? magnet_closure_height : 0)
            ])
          cylinder(d=get_magnet_hole_diameter(),
                   h=get_magnet_hole_height() + (magnet_generate_closure ? 0 : SWO));
      }
    }
}
...
module magnet_holder_box(clearance) {
  magnet_holder(height = get_box_height_outside(),
                wall_distance = get_magnet_holder_radius() + get_magnet_holder_rounding_fix_offset(),
                wall_rounding = get_lip_rounding(),
                clearance = clearance);
}

module magnet_holders_box_pair(clearance) {
  magnet_holder_box_left_x_position = -(
    (get_x_width_outside() + get_box_x_margin())
    -get_magnet_holder_radius()
    -thickness
    -get_lp_tolerance()
  );
  magnet_holder_box_right_x_position = -(
    get_box_x_margin() + get_magnet_holder_radius() + thickness + get_lp_tolerance()
  );
  y_position = -get_y_depth()/2 - get_lp_thickness() + get_magnet_holder_radius();
  // When using hinges with 2 magnets, place both on the front side
  left_y_position = (get_generate_hinges() && magnets_number == 2)
                    ? y_position
                    : -y_position;
  left_rotation = (get_generate_hinges() && magnets_number == 2) ? 90 : 0;
...
module magnet_holder_lid(clearance) {
  xflip()
    magnet_holder(height = get_lid_height_outside() - get_lp_height(),
                  wall_distance = get_magnet_holder_radius() +
                                  get_lp_tolerance() +
                                  get_magnet_holder_rounding_fix_offset(),
                  wall_rounding = get_lid_cut_out_rounding(),
                  clearance = clearance);
}

module magnet_holders_lid_pair(clearance) {
  magnet_holder_lid_right_x_position = (get_x_width_outside() + get_lid_x_margin())
                                      -get_magnet_holder_radius()
                                      -thickness
                                      -get_lp_tolerance();
  magnet_holder_lid_left_x_position = get_lid_x_margin() +
                                      get_magnet_holder_radius() +
                                      thickness +
                                      get_lp_tolerance();
  y_position = -get_y_depth()/2 -get_lp_thickness() + get_magnet_holder_radius();
  // When using hinges with 2 magnets, place both on the front side
  right_y_position = (get_generate_hinges() && magnets_number == 2)
                     ? y_position
                     : -y_position;
  right_rotation = (get_generate_hinges() && magnets_number == 2) ? -90 : 0;
...
// Main magnets module - generates all magnet holders
module magnets() {
  ...
  if (get_generate_magnets()) {
    ...
    required_height = get_magnet_hole_height() + (magnet_generate_closure ? magnet_closure_height : 0);

    assert(get_box_height_inside() >= required_height,
           str("Not enough height in box for magnets. ",
               "Box height: ", bottom_height, "mm, ",
               "required: ", required_height, "mm"));

    lid_available_height = lid_height - get_lp_height();
    ...
    assert(magnet_holder_diameter > get_magnet_hole_diameter(),
           str("Magnet holder diameter must be larger. ",
               "Magnet holder diameter: ", magnet_holder_diameter, "mm, ",
               "required more than: ", get_magnet_hole_diameter(), "mm"));
  }
