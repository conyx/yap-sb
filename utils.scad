...
function parse_compartments_grid(compartments_flat) =
  let(
    ...
    _zero_pos = [for (i = [0:len(compartments_flat)-1]) if (compartments_flat[i] == 0) i],
    _a4 = assert(len(_zero_pos) < 2
             ? true
             : len([for (i = [0:len(_zero_pos)-2])
                    if (_zero_pos[i+1] - _zero_pos[i] < 3) i]) == 0,
           str("Between two zeros in compartments_dimensions there must be 2 or more non-zero values. ",
               "Zero positions: ", _zero_pos)),
